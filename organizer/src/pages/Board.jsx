...
const Board = () => {
  ...
  const boards = useSelector((state) => state.board.value) || []  // Ensure boards is not undefined
  const favouriteList = useSelector((state) => state.favourites.value) || []  // Ensure favouriteList is not undefined
    ...
    const getBoard = async () => {
      try {
        ...
        if (res) {
          ...
          setIsFavourite(res.favourite || false)
          setIcon(res.icon || '')
        }
      } catch (err) {
        alert(err)
  ...
  const onIconChange = async (newIcon) => {
    ...
    if (index === -1) return; // Handle case where boardId is not found
  ...
  return (
    <>
      ...
      <Box sx={{ padding: '10px 50px' }}>
        ...
        <Box>
          {/* Kanban board */}
          <Kanban data={sections} boardId={boardId} showClosingDate={true} />
        </Box>
      </Box>
    </>
  )
